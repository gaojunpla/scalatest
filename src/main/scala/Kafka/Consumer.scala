...
class Consumer(val consumer:String, val stream: KafkaStream[Array[Byte], Array[Byte]]) extends Runnable {
...
object KafkaConsumerTest{
  def main(args: Array[String]): Unit = {
    ...
    props.put("zookeeper.connect", "master:2181,clone:2181,clone2:2181")
    ...
    for (i <- 0 until stream.size){
      pool.execute(new Consumer(s"Consumer: $i", stream.get(i)))
    }
  }
}
